...
public class Server {
    ...
    final private ConcurrentHashMap<String, Map<String, Handler>> handlers;

    static Socket socket;


    public Server(int port, int numberOfThreads) {
        exService = Executors.newFixedThreadPool(numberOfThreads);
    ...
    private void connection(Socket socket) {
        ...
        while (true) {
            try (InputStream inputStream = socket.getInputStream();
                ...
                Map<String, Handler> path = handlers.get(requestLine != null ? requestLine.method() : null);
                if (path == null) {
